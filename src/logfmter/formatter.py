...
class Logfmter(logging.Formatter):
    ...
    def flatten_dict(self, v: dict, root: str = "") -> dict[str, Any]:
        ...
        for key, value in v.items():
            key = f"{root}.{self.normalize_key(key)}" if root else self.normalize_key(key)
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record.exc_info and "exc_info" not in self.ignore_keys:
